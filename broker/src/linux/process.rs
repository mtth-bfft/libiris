...
use iris_ipc::{MessagePipe, CrossPlatformMessagePipe, IPCMessagePipe, IPCRequestV1, IPCResponseV1, IPCVersion};
use libc::c_int;
...
pub(crate) struct OSSandboxedProcess {
    ...
    seccomp_trap_bpf: RwLock<Option<Vec<u8>>>,
    // Boolean flipped to true once the worker has run execve() when seccomp
    // user notifications are in use. Meaningless variable otherwise.
    seccomp_unotify_execve_ran: RwLock<bool>,
}
...
impl CrossPlatformSandboxedProcess for OSSandboxedProcess {
    fn new(
        ...
        let can_read_worker_mem = match File::open(format!("/proc/{}/mem", worker_pid)) {
            ...
            Err(e) => {
                println!(" [!] Cannot use seccomp mitigations: unable to open worker process memory, {}", e);
                false
            },
        };
        ...
        let proc_mem = if can_read_worker_mem {
            match File::open(format!("/proc/{}/mem", worker_pid)) {
                ...
                Err(e) => {
                    ...
                    return Err(format!("Unable to open process memory after execve(): {}", e));
                },
            }
        } else {
            None
        ...
        let process = Arc::new(Self {
            ...
            seccomp_trap_bpf: RwLock::new(None),
            seccomp_unotify_execve_ran: RwLock::new(false),
        });

        // Now that we can pass Arc<> references to our Process struct, start serving IPC
        // in a detached thread holding an Arc<> to read our memory. Note: this is an OS-specific
        // code to allow processes with seccomp-notify to close their IPC socket after late
        ...
        *(process.ipc_thread.write().unwrap()) = Some(std::thread::spawn(move || {
            loop {
                ...
                let (resp, handle) = match req {
                    IPCRequestV1::Syscall { arch, nr, arg1, arg2, arg3, arg4, arg5, arg6 } => handle_syscall(arch, nr, arg1, arg2, arg3, arg4, arg5, arg6, &processref),
                    other => {
        ...
        // Wait for the worker to tell us if it succeeded in applying a seccomp filter, so that we can
        // spawn a seccomp-unotify thread if necessary, or prepare a seccomp-trap filter for later.
        match sock_seccomp_parent.recv_with_handle() {
            Ok((_, Some(notify_fd))) => {
                ...
                *(process.seccomp_unotify_thread.write().unwrap()) = Some(std::thread::spawn(move || {
                    ...
                    loop {
                        ...
                        let (code, handle) = handle_syscall(arch, nr, arg1, arg2, arg3, arg4, arg5, arg6, &processref);
            ...
            Ok((v, None)) => {
                ...
                println!(" [!] Cannot use seccomp user notification mitigation: error {}", error);
            },
            Err(e) => {
                println!(" [!] Cannot use seccomp user notification mitigation: {}", e);
            },
        };

        // If the worker was supposed to try seccomp user notification and failed,
        // fallback on seccomp-trap
        *(process.seccomp_trap_bpf.write().unwrap()) = match (&process.proc_mem, process.seccomp_unotify_thread.read(), generate_seccomp_filter(SCMP_ACT_TRAP)) {
            (None, _, _) => {
                None
            },
            ...
            (_, _, Err(e)) => {
                println!(" [!] Cannot use seccomp trap mitigation: unable to compile filter ({})", e);
                None
            },
            (Some(proc_mem), _, Ok(filter)) => {
                ...
                if let Some(mut tmp_fd) = tmp_fd {
                    ...
                    if res != 0 {
                        println!(" [!] Unable to clean up tmp file {}", std::str::from_utf8(&tmp_fd_path).unwrap_or("<?>"));
                    }
                    ...
                    if res != 0 {
                        ...
                        if let Err(e) = tmp_fd.seek(SeekFrom::Start(0)) {
                            println!(" [!] Cannot use seccomp-trap mitigation: lseek() failed ({})", e);
                            None
                        }
                        else {
                            let mut bpf = vec![];
                            if let Err(e) = tmp_fd.read_to_end(&mut bpf) {
                                println!(" [!] Cannot use seccomp-trap mitigation: read() failed ({})", e);
                                None
                            }
                            else {
                                Some(bpf)
                    ...
                    println!(" [!] Cannot use seccomp-trap mitigation: mkstemp() failed (errno {})", Error::last_os_error().raw_os_error().unwrap_or(0));
                    None
                }
            }
        };
        println!(" [+] Process PID={} execve() successful, running", process.get_pid());
        Ok(process)
    }
...
impl OSSandboxedProcess {
    pub fn read_cstring_from_ptr(&self, ptr: u64, max_len: usize) -> Result<CString, String> {
        ...
        match bytes.iter().position(|b| *b == 0) {
            None => return Err(format!("No null byte found in the first {} bytes at 0x{:X}", max_len, ptr)),
            Some(pos) => bytes.truncate(pos),
        };
        let res = match CString::new(bytes) {
            Ok(s) => s,
            Err(e) => return Err(format!("Invalid C string read from worker memory at address 0x{:X}: {}", ptr, e)),
        };
        Ok(res)
    }
}
...
extern "C" fn process_entrypoint(args: *mut c_void) -> c_int {
        ...
        if let Ok(filter) = generate_seccomp_filter(SCMP_ACT_NOTIFY) {
            ...
            if res != 0 {
                ...
                if let Err(e) = args.sock_seccomp.send(&[], Some(&notify_fd)) {
                    eprintln!("Fatal error: unable to send seccomp notify handle to broker: {}", e);
                    std::process::abort(); // don't panic!() which unwinds
                }
            }
        }
    ...
    if let Err(e) = args.sock_execve_errno.send(&errno_bytes, None) {
        eprintln!("Fatal error: failed to report execve() error to broker: {}", e);

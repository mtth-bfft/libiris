...
impl CrossPlatformSandboxedProcess for OSSandboxedProcess {
    fn new(policy: &Policy, process_config: &ProcessConfig) -> Result<Self, BrokerError> {
        ...
        child_pipe.set_inheritable(false)?;
        ...
        let entrypoint_params = EntrypointParameters {
            debug_fd: Some(libc::STDERR_FILENO),
        ...
        // Note 1: PID namespace needs to be created during clone(): using unshare()
        // later would only apply to child processes and not to the worker process.

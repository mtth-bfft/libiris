...
pub(crate) fn serialize<'a, T: Serialize>(
    msg: &'a T,
    buffer: &'a mut [u8],
) -> Result<&'a mut [u8], IpcError<'static>> {
    postcard::to_slice(msg, buffer).map_err(|e| IpcError::InternalSerializationError {
        description: match e {
            ...
            postcard::Error::NotYetImplemented => {
                "tried to use a feature not supported by postcard"
            }
            postcard::Error::SerializeBufferFull => "message too long",
            postcard::Error::SerializeSeqLengthUnknown => {
                "postcard requires sequence length to be known"
            }
    ...
    postcard::from_bytes(buffer).map_err(|e| IpcError::InternalDeserializationError {
        ...
        description: match e {
            ...
            postcard::Error::DeserializeBadVarint => {
                "postcard found a varint that did not terminate"
            }
            postcard::Error::DeserializeBadBool => "postcard found a bool that was neither 0 nor 1",
            postcard::Error::DeserializeBadChar | postcard::Error::DeserializeBadUtf8 => {
                "postcard found an invalid unicode char"
            }
            postcard::Error::DeserializeBadOption => {
                "postcard found an option discriminant that was neither 0 nor 1"
            }
            postcard::Error::DeserializeBadEnum => {
                "postcard found an enum discriminant that was >u32max"
            }
